...
RequestSession.prototype.getBoard = async function(boardName) {
  ...
  if(300 <= response.status && response.status < 400) {
    return this.fromUrl('https:' + response.headers.get('Location'));
  } else if(response.status < 300) {
    return this.fromUrl(primaryBoardUrl);
  } else {
    throw new Error('Cannot get that board name');
  }
}
